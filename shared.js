...
const LEADERS=[
  {id:'kendra',name:'Kendra Burris',title:'Chief Executive Officer',kpis:[
  ...
  {id:'megan',name:'Megan Cockman',title:'Chief Financial Officer',kpis:[
  ...
  {id:'chris',name:'Chris Farr',title:'Chief Operating Officer',kpis:[
  ...
  {id:'zac',name:'Zac Cockman',title:'Chief Growth Officer',kpis:[
  ...
  {id:'aubrey',name:'Aubrey Stutler',title:'General Counsel',kpis:[
  ...
  {id:'kyle',name:'Kyle Lair',title:'Chief People Officer',kpis:[
  ...
  {id:'brian',name:'Brian Thatcher',title:'Chief Development Officer',kpis:[
...
// ── DATA ──
async function loadKpiData(year, scopeLeaderId){
  let q = db.collection('kpiData').where('year','==',String(year));
  if(scopeLeaderId) q = q.where('leaderId','==',scopeLeaderId);
  const snap = await q.get();
  return snap.docs.map(d=>d.data());
}

async function saveKpiRows(year, rows, updatedBy){
  const batch = db.batch();
  const now = new Date().toISOString();
  rows.forEach(r=>{
    const id = year+'_'+r.leaderId+'_'+r.kpiIndex;
    const ref = db.collection('kpiData').doc(id);
    batch.set(ref, {
      year:String(year), leaderId:r.leaderId, leaderName:r.leaderName, kpiIndex:r.kpiIndex,
...
async function loadNotes(year, leaderId){
...
async function saveNote(year, leaderId, kpiIndex, role, author, text){
  await db.collection('notes').add({ year:String(year), leaderId, kpiIndex, role, author, date:new Date().toISOString(), text });
}

async function loadMisses(year, leaderId){
...
async function saveMiss(year, leaderId, kpiIndex, description, deduction, loggedBy){
  await db.collection('misses').add({ year:String(year), leaderId, kpiIndex, date:new Date().toISOString(), description, deduction, loggedBy });
}

async function getEbitda(year){
  const doc = await db.collection('settings').doc(String(year)).get();
  return doc.exists ? Number(doc.data().ebitda) : 10;
}

async function setEbitda(year, ebitda){
  await db.collection('settings').doc(String(year)).set({ ebitda });
}
